...
def main():
    ...
    # Increase the maximum position embedding to accept longer input sequence
    if args.long:
        sd = model.state_dict()
        shorter_pos_embeds = sd['model.encoder.embed_positions.weight']
        new_config = model.config
        new_config.max_position_embeddings = args.max_input_length
        new_model = init_model_from_config(new_config, device=args.device)
    ...
    args.block_size = model.config.max_position_embeddings # if "bart" in args.model_name_or_path else tokenizer.max_len_single_sentence
...
#    args.max_input_length = model.config.max_position_embeddings 
    ...
    if args.do_train:
        ...
        args.block_size = args.max_input_length
        model.to(args.device)
