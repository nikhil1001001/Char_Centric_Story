...
class TextDataset(Dataset):
    ...
    def __init__(self, tokenizer, args, file_path="train", block_size=512):
        ...
        filename = f"{args.model_name_or_path}_cached_{args.max_input_length}_{name_pos}_{filename}"
        ...
        if os.path.exists(cached_features_file) and not args.overwrite_cache:
            ...
            inputs, max_input_length = self.__truncate__(
                [process(ex[0]) for ex in examples], args.max_input_length - 1
            ) # -1 for reserved for [desc] token
            outputs, max_output_length = self.__truncate__(
                [process(ex[-1]) for ex in examples], args.max_output_length - 1
            ) # -1 for reserved for <eos> token

            # Add [desc] to truncated input
            desc_special_token_id = tokenizer.convert_tokens_to_ids("[desc]" if args.task=="generative" else "[name]")
            inputs = self.__add_eos__(inputs, desc_special_token_id)

            # Add eos to truncated output
            eos_token_id = tokenizer.convert_tokens_to_ids(tokenizer.eos_token)
            outputs = self.__add_eos__(outputs, eos_token_id)
    ...
    # def __truncate__(self, lst, max_length, lst2=None):
    #     # reserved length for the optional second input
    #     effective_max_len = max_length - max([len(item) for item in lst2]) if lst2 else max_length
    #     max_length = min(effective_max_len, max([len(item) for item in lst]))
    #     lst = [item[:max_length] for item in lst]
    #     return lst, max_length
    ...
    def __add_eos__(self,lst, eos_token_id):
        lst = [
            item + [eos_token_id]
            for item in lst
        ]
...
def main():
    ...
    if args.do_train and not args.continue_training:
        special_tokens = ["[name]", "[sum]", "[desc]", "<eos>"]
